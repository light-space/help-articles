...
> **What is this page about:** What to do when a third-party integration (CRM, Slack, payment processor, or other connected service) isn't syncing correctly, how to read each integration's connection status, sync schedules and manual-sync rules, common errors like duplicate entries or currency mismatches, and edge cases such as stalled syncs, records deleted at the source, and silent connection expiry.
...
- Issue 4: Duplicate Entries After Sync
...
4. **Check Bot Permissions in Slack (admin required)**:
   - Go to your Slack workspace settings (admin required)
...
1. **Verify Sync Is Enabled**:
   - Go to **Settings** > **Integrations** > **Salesforce** or **HubSpot**
...
3. **Verify API Connection**:
   ...
   - Click **Delete connection**, then reconnect and sign in with your CRM account again
   - This effectively re-authenticates the integration
4. **Check Permissions**:
...
5. **Review Workflow Runs**:
   ...
   - Go to **Settings** > **Workflows** and open the relevant workflow to review its is published and it runs
...
3. **Refresh OAuth Token**:
   ...
   - Re-authenticate (Slack, Teams, HRM) or delete and reconnect (other integrations)
...
## Issue 9: Sync Stalls or Records Don't Sync, No Error Shown
...
## Issue 10: Deleted Records Still Appear in Light

> Applies to: Salesforce, HubSpot
> Note: This behavior is still being confirmed with engineering before publishing. Two separate checks gave different answers on whether source-side deletions propagate to Light, so please get a final sign-off from the team before this issue goes live.
...
1. **Understand the Current Behavior**:
   ...
   - If this is confirmed, a deletion in the CRM will not automatically remove the record from Light
2. **Remove the Record Manually**:
   - Delete the record directly in Light if it's no longer needed
   - If there's no automatic cleanup for source-side deletions, treat this as an expected limitation rather than a sync error

## Issue 11: Integration Silently Stops Working
...
1. **Check for an Expired Connection**:
   ...
   - Go to **Settings** > **Integrations** and check the integration's status for an expired, inactive, or action-required state
...
## Issue 12: Second Slack Workspace Overwrites the First
...
2. **Reconnect the Workspace You Need**:
   - If you need to switch back, go to **Settings** > **Integrations** > **Slack** and re-authenticate with the workspace you need
   - If you need Light active in multiple Slack workspaces simultaneously, contact support to discuss options

## Issue 13: Syncs or Notifications Fail Under Heavy Load
...
- [Common issues and solutions](https://light.inc/help/troubleshooting/common-issues)
- [Bank connection troubleshooting](https://light.inc/help/troubleshooting/bank-connection-issues)
- [Data import errors](https://light.inc/help/troubleshooting/data-import-errors)
- [Contacting Light support](https://light.inc/help/troubleshooting/contacting-support)
