...
3. Fill in the user's details:
   ...
   - **Email** address
   - **Country** (required)
   - **State**, **City**, **Address**, and **Zip / postcode** (optional)
   - **Phone number** (optional)
4. Select their **Entity** (required, the entity they'll work in primarily)
...
Detail and role changes save immediately, but role changes only take full effect the next time the user logs in. If you need a role change to apply right away, ask the user to log out and back in.

## Deactivating Users

To remove a user's access, deactivate their account:

1. Go to **Business partners → Users**
2. Find the user and click their name to open their details
3. In the dialog footer, turn off the **Active** toggle
4. Click **Save**

Deactivated users cannot log in, but their historical data and audit trail remain in the system for compliance purposes.

> Important: Consider whether you need to reassign any workflows, approvals, or manager relationships before deactivating a user.
...
- **Deactivate instead of ignoring**: Deactivate users who no longer need access to maintain a clean user list.
